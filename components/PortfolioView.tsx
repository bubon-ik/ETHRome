...
export const PortfolioView: React.FC<PortfolioViewProps> = ({ className = "" }) => {
  ...
  const { getPortfolioData, isLoading, error, clearError } = usePortfolio();
  ...
  const loadPortfolio = async () => {
    ...
    try {
      ...
      if (!data) {
        console.log('Portfolio API недоступен, используем fallback данные');
        setPortfolioData({
          tokens: [],
          summary: {
            totalValue: 0,
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="font-medium text-yellow-800 dark:text-yellow-200">
                Portfolio API Limited
              </h4>
              <p className="text-sm text-yellow-700 dark:text-yellow-300">
                1inch Portfolio API is not available for Base network. Showing fallback data.
              </p>
            </div>
          </div>
        </div>
